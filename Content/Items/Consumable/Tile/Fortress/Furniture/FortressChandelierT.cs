...
namespace QwertyMod.Content.Items.Consumable.Tile.Fortress.Furniture
{
    public class FortressChandelierT : ModTile
    {
        ...
        {
            Item.NewItem(i * 16, j * 16, 48, 32, ItemType<FortressChandelier>());
        ...
        public override void PostDraw(int i, int j, SpriteBatch spriteBatch)
        {
            ...
            for (int k = 0; k < 7; k++)
            {
                ...
                Main.spriteBatch.Draw(Request<Texture2D>("QwertyMod/Content/Items/Consumable/Tile/Fortress/Furniture/FortressChandelier_Flame").Value, new Vector2((float)(i * 16 - (int)Main.screenPosition.X + offsetX) - (width - 16f) / 2f + x, (float)(j * 16 - (int)Main.screenPosition.Y + offsetY) + y) + zero, new Rectangle(frameX, frameY, width, height), color, 0f, default(Vector2), 1f, SpriteEffects.None, 0f);
            }
        }
    }
}